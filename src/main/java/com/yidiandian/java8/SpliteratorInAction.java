...
public class SpliteratorInAction {
    ...
    public static void main(String[] args) {
       ...
       /* IntStream intStream = IntStream.rangeClosed( 0,10 );
        Spliterator.OfInt spliter = intStream.spliterator();
        Consumer<Integer> consumer = i -> System.out.println(i);
        spliter.forEachRemaining( consumer);*/

       MySpliteratorText resText = new MySpliteratorText( text);
       resText.stream().count();

       resText.stream().forEach( System.out::println );
       resText.stream().filter( s-> !s.equals( "" ) ).forEach( System.out::println );
